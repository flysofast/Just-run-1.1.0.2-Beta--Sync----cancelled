...
namespace Map
{
    public partial class App : Application
    {
        ...
        public App()
        {
            ...
            try
            {
                ...
                if (!settings.Contains("FirstRunAndNotRegistered"))
                {
                    ...
                    settings.Add("UserInfo", userInfo);
                    ...
                    settings.Add("RunCount", 0);
                    settings.Add("LaunchCount", 0);
                    settings.Add("NewRelease", true);
                }
                else
                    if (!settings.Contains("RunCount")) //UPDATE from 1.0.0.2 to 1.1.0.0
                    {
                        settings.Add("RunCount", 0); //TRUE or NOT FOUND means first run and haven't registered yet while   
                        settings.Add("LaunchCount", 0);
                        settings.Add("NewRelease", true);
                        // FALSE means IT"S NOT FIRST RUN & ALREADY REGISTERED!!
                        var Data = (List<ARunData>)settings["RunData"];
                        List<ARunData> backupData = new List<ARunData>();
                        if (Data != null)
                            foreach (ARunData item in Data)
                                backupData.Add(item);
                        if (backupData != null)
                            foreach (ARunData item in backupData)
                            {
                                ARunData a = new ARunData();
                                a = item;
                                Data.Remove(item);
                                a.Save();
                            }
        ...
        {
            IsolatedStorageExplorer.Explorer.Start("169.254.80.80");
            //IsolatedStorageSettings settings = IsolatedStorageSettings.ApplicationSettings;
